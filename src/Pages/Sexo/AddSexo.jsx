...
const AddSexo = ({ refreshData }) => {
    ...
    const handleSubmit = async (values, { resetForm }) => {
        try {
            const response = await postData(`${import.meta.env.VITE_APP_ENDPOINT}/sexos`, values, token);
            ...
            if (response.status) {
                ...
                CustomSwal.fire(
                    'Agregado',
                    'El sexo ha sido agregado correctamente.',
                    'success'
                );
                ...
                console.error('Error al agregar el sexo: ', response.error.response.data.error);
                CustomSwal.fire({
                    icon: 'error',
        ...
        <>
            ...
            <CustomModal Open={Open} setOpen={setOpen} handleClose={handleClose}>
                <div className="flex items-center mb-2">
                    ...
                    <h1 className='text-lg font-bold fl'>Añadir un Sexo</h1>
                </div>
                <Formik
