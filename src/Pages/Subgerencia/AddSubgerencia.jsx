...
const AddSubgerencia = ({refreshData}) => {
    ...
    const handleSubmit = async (values, { setSubmitting, resetForm }) => {
        try {
            const response = await postData(`${import.meta.env.VITE_APP_ENDPOINT}/subgerencias`, values, token);
    
            if (response.status) {
                setOpen(false);
                CustomSwal.fire(
                    'Agregado',
                    'La subgerencia ha sido agregado correctamente.',
                    'success'
                );
                refreshData();
                resetForm();
                ...
                swalError({
                    message: 'Ocurrió un error al agregar la subgerencia',
                    data: erroresArray,
                });
            }
        } catch (error) {
            ...
            swalError({
                message:  'Error inesperado al agregar la subgerencia',
                data: [error.message],
            });
        } finally {
        ...
        <>
            ...
            <CustomModal Open={Open} setOpen={setOpen} handleClose={handleClose}>
                <div className="flex items-center mb-2">
                    ...
                    <h1 className='text-lg font-bold fl'>Añadir una Subgerencia</h1>
                </div>
                <Formik
