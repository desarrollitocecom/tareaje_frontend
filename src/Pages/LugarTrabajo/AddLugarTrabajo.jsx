...
const AddLugarTrabajo = ({ refreshData }) => {
    ...
    const handleSubmit = async (values, { setSubmitting, resetForm }) => {
        try {
            ...
            const response = await postData(`${import.meta.env.VITE_APP_ENDPOINT}/lugarestrabajos`, values, token);
    
            if (response.status) {
                setOpen(false);
                CustomSwal.fire(
                    'Agregado',
                    'El lugar de trabajo ha sido agregado correctamente.',
                    'success'
                );
                refreshData();
                resetForm();
                ...
                swalError({
                    message: 'Ocurrió un error al agregar el lugar de trabajo',
                    data: erroresArray,
                });
            }
        } catch (error) {
            ...
            swalError({
                message:  'Error inesperado al agregar el lugar de trabajo',
                data: [error.message],
            });
        } finally {
        ...
        <>
            ...
            <CustomModal Open={Open} setOpen={setOpen} handleClose={handleClose}>
                <div className="flex items-center mb-2">
                    ...
                    <h1 className='text-lg font-bold fl'>Añadir un Lugar de Trabajo</h1>
                </div>
                <Formik
...
export default AddLugarTrabajo